locals {
  ...
  project_all_repositories = [
    for repo in var.gcr_repositories : {
      ...
      filter                    = repo.parameters != null ? "grace-${repo.parameters.grace != null ? repo.parameters.grace : "0"}-keep-${repo.parameters.keep != null ? repo.parameters.keep : "0"}-repo_skip_filter-${repo.parameters.repo_skip_filter != null ? repo.parameters.repo_skip_filter : "no"}-repo_prefix_filter-${repo.parameters.repo_prefix_filter != null ? repo.parameters.repo_prefix_filter : "no"}-tag_keep_filter-${repo.parameters.tag_keep_filter != null ? repo.parameters.tag_keep_filter : "no"}-tag_filter-${repo.parameters.tag_filter != null ? repo.parameters.tag_filter : "no"}-tag_filter_any-${repo.parameters.tag_filter_any != null ? repo.parameters.tag_filter_any : "no"}-tag_filter_any-${repo.parameters.tag_filter_any != null ? repo.parameters.tag_filter_any : "no"}${repo.parameters.dry_run != null ? (repo.parameters.dry_run ? "-dry_run" : "") : ""}" : "delete-all-untagged-images-recursive"
      ...
      for repo in gcr.repositories : merge(repo,
        {
          ...
          filter                    = "grace-${repo.grace != null ? repo.grace : "0"}-keep-${repo.keep != null ? repo.keep : "0"}-repo_skip_filter-${repo.repo_skip_filter != null ? repo.repo_skip_filter : "no"}-repo_prefix_filter-${repo.repo_prefix_filter != null ? repo.repo_prefix_filter : "no"}-tag_keep_filter-${repo.tag_keep_filter != null ? repo.tag_keep_filter : "no"}-tag_filter-${repo.tag_filter != null ? repo.tag_filter : "no"}-tag_filter_any-${repo.tag_filter_any != null ? repo.tag_filter_any : "no"}-tag_filter_all-${repo.tag_filter_all != null ? repo.tag_filter_all : "no"}-recursive-${repo.recursive != null ? repo.recursive : false}${repo.dry_run != null ? (repo.dry_run ? "-dry_run" : "") : ""}"
